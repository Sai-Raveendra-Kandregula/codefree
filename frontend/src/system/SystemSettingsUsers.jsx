...
function SystemSettingsUsers({ pendingUsers = false }) {
    ...
    const COLUMNS = [
        ...
        { label: 'Email', renderCell: (item) => <span title={item['email']}>{item['email']}</span> },
        {
            ...
            renderCell: (item) => (
                ...
                >
                    Created by <UserLink admin_url={true} user_id={item['created_by']} />
                    <br />
        ...
        },
        {
            ...
            renderCell: (item) => (
                ...
                >
                    Modified on {new Date(item['updated_on']).toLocaleDateString()}
                </div>
            ),
        },
        {
                ...
                >
                    ...
                    <IconButton
                        ...
                        onClick={(e) => {
                            ...
                            if (
                                window.confirm(
                                    `Do you want to delete : "${item['display_name']}" (${item['user_name']}) ?`,
                                )
                            ) {
                                User.deleteUser(item['user_name'])
                                    .then((out) => {
    ...
    return (
        <CFPage title={pendingUsers ? 'Pending Users' : 'Users'}>
            <div
                style={{
                    width: '100%',
                    display: 'flex',
