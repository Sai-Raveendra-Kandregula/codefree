...
from modules.server.database import engine, get_db_session
...
@projectsRouter.get("/project/-/all")
...
@projectsRouter.get("/project/{slug}")
def get_project(
...
def get_project_report(
    ...
    try:
        ...
        with open(
            os.path.join(getProjectReportsPath(project), report_data.report_path)
        ) as fp:
...
def upload_project_report(
    ...
    report: Report = (
        db_session.query(Report)
        .where(Report.id.is_(report_id))
        .where(Report.project_id.is_(project.id))
        .scalar()
    )
...
def export_project_report(
    ...
    try:
        with open(
            ...
            CheckingModule.set_output(issue_items_cls)
            base_filename = (
                f'report_{project}_{report}_{report_ts.strftime("%Y%m%d_%H%M%S")}'
            )
            ...
            try:
                ...
                return {"message": f"Error Converting report into {format} format"}
