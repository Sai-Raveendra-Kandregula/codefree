...
from modules.server.common import logger, APP_NAME, AUTHOR, CONTACT_EMAIL, APP_DATA_PATH, APP_CONF_FILE, ROOT_PATH, ROOT_PATH_MID_URL


def serve_codefree_backend(app:FastAPI):
    app.include_router(usersRouter, prefix=f'{ROOT_PATH_MID_URL}/api')
    app.include_router(projectsRouter, prefix=f'{ROOT_PATH_MID_URL}/api')
    app.include_router(gitlabRouter, prefix=f'{ROOT_PATH_MID_URL}/api')
    ...
    @app.get(f"{ROOT_PATH_MID_URL}/api/greetings")
